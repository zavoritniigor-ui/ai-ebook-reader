...
- **Red CI on `d1a9b06`** (`tests/ai_contract_browser.py`, HVAC tap never opened the tooltip in CI) could NOT be reproduced locally under CI's Chrome flags, CI test ordering, 16× CPU throttling or slow network. The test's tap helper now (1) waits for `pdfContinuousReady` like the other PDF suites, (2) scrolls the word into view and waits for stable geometry, (3) closes any Practice panel/drawer left open by an earlier section, (4) retries a tap like a user would, and (5) on failure reports what was under the pointer/viewport/viewer state. Two real hazards were demonstrated with it (a word aimed above the viewport; a Practice panel covering the reader) — either would make a tap miss on a machine where layout differs; whether one of them was the CI cause is unproven until CI runs.
