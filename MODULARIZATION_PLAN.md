...
ai-client.js	dataUrlMime, visionViaGemini, visionViaGroq, callAIVision, aiAvailable, callAI, GROQ_MODEL, GROQ_VISION_MODEL — ПОПРАВКА при фактичному перенесенні (Step 3): startAiTask НЕ переноситься сюди. У коді він фізично затиснутий між els.grammarTab.onclick/els.askTab.onclick (панельне wiring), а не поруч із callAI-кластером (той — окремий суцільний блок далі у файлі) — переносити його означало б розривати сусідній UI-wiring код без потреби. startAiTask відкладено до grammar-svo.js (Step 8), де він і використовується найчастіше (режими grammar/ask/level).
