...
| `learning-stats.js` | Compact current-page reading/vocabulary statistics: Unicode word segmentation, current-page visibility filtering, exact source-occurrence tap tracking, per-book local persistence, conservative cached EN/FR CEFR lookup with an explicit unclassified bucket, and the toolbar popover renderer. Records use source offsets within a PDF page or reflowable chapter/block so pagination changes do not create a second tracking system. No network/AI request is made for statistics. |
...
| `tests/learning_stats_languages_grammar_browser.py` | Current-page word/tap statistics and persistence isolation, CEFR unknown handling, compact stats UI, new language registry/detection/TTS/offline fallbacks, grammar-rule prompt contracts for French/English, and real Grammar/Ask panel response rendering. |
