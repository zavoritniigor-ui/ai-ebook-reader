...
| `grammar-svo.js` | The Grammar/Ask AI panels: `startAiTask` (Ask/Language-level still stream free text through `safeHtml`; `mode==='grammar'` early-dispatches to `runGrammarAnalysis`), SVO sentence-part analysis (`analyzeSVO`/`applySVOParts`), and the **contextual Verbs/Adjectives Grammar panel**. One structured-JSON AI call (`buildGrammarAnalysisPrompt`, task `grammar_analysis`) detects verbs AND adjectives together; `normalizeGrammarAnalysis` is the single validation gate — it drops items whose `surface` is not literally in the analyzed text (fabrication guard), rejects unknown POS, de-duplicates lemma+surface, and routes every feature/form through `normalizeGrammarFeatures`/`GRAMMAR_LANG_CONFIG` (`core.js`) so no category irrelevant to the language can reach the UI. The result is cached (`grammarAnalysisCache`, bounded FIFO, keyed by book+source language+explanation language+text), so switching Verbs↔Adjectives (`switchGrammarMode`) or re-tapping the same passage never re-sends the source. `renderGrammarPanel` groups occurrences by lemma into cards; `focusGrammarItem(item, lang)` renders one occurrence's detail (badges, stem+ending only when the model's split reconstructs the surface exactly, paradigm/agreement grid, highlighted sentence, "why this form") and is the SHARED entry point Practice calls when a highlighted word is clicked. Tense/mood controls come from `GRAMMAR_LANG_CONFIG[lang].verb.tenses` and are cleared in Adjectives mode; `selectGrammarTense`→`fetchVerbParadigm` does a small cached per-lemma+tense lookup (task `grammar_paradigm`). Everything AI-supplied is inserted via `createElement`/`textContent` — never `innerHTML`. Stale protection: `beginAsyncTask('grammar')` plus a post-`await` `task.current()` check (deliberately tested with a mock that ignores the abort signal). `resetGrammarState()` (called by `openBookFile`) clears analysis, focus and cache on book change. |
...
session guards. `validatePracticeReading` throws on structural problems (missing
title/language, HTML/script injection, >8 paragraphs, or a passage below
`MIN_READING_CHARS` — the "not two trivial sentences" floor) but silently DROPS an
individual bad target (surface not literally in its paragraph, unsafe text,
duplicate paragraph+surface) so one bad occurrence cannot discard a good passage.
...
only; each target's own occurrence is wrapped in a `.practice-target` button, and
clicking it calls `focusGrammarItem` (`grammar-svo.js`) with data already in hand —
