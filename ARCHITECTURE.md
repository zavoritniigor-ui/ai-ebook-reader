...
| `google-classroom.js` | Google sign-in (Google Identity Services token client) plus read-only Google Classroom/Drive REST calls: course list → one card per coursework/material (title, Assignment/Material badge, due-or-published date, short description, Drive attachments listed directly under the card — no separate attachments screen) → `main.js`'s `openBookFile` opens the fetched (or, for native Google Docs/Slides, PDF-exported) file directly, no manual download. Requires `drive.readonly` instead of `drive.file` because `drive.file` returns 404 for teacher-created Classroom attachments. Includes `supportsAllDrives=true` for Shared Drive support. Owns the `#classroom-modal` UI and its own module-local state (`googleAuth`, `classroomStack`) — no globals beyond calling `openBookFile`. |
...
| `tests/google_classroom_browser.py` | `google-classroom.js`'s full sign-in → courses → one card per coursework/material (title, Assignment/Material badge, due-or-published date, optional short description, attachments listed directly under the card) → attachment → open-in-Reader flow, with Google Identity Services and the Classroom/Drive REST calls mocked (real `accounts.google.com` network-blocked so the mock is never raced by a real script load). Asserts the requested OAuth scope is exactly the minimal read-only set (and never includes `drive.readonly` or the teacher-only `classroom.coursework.students.readonly`), that a native Google Doc is exported to PDF rather than fetched as-is, that an already-supported format (PDF/TXT) is fetched directly via `alt=media`, that a card with no description simply omits that line, that non-Drive materials (links/videos/forms) never appear as openable attachments, that the opened file really renders (PDF text layer, `detectLang` on the new content), and that the modal participates correctly in the Android-Back overlay stack. |
