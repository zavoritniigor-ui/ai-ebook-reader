...
occurrence statistics model/UI, the fixed 100-word A–K help-coverage matrix,
overlapping span deduplication, page leave/return persistence, tokenization and
Ask/Grammar routing, plus added languages and the French/English grammar-rule
prompt contract. It uses the same local server and headless Chrome session.

`learning_stats_position_independence_browser.py` proves the same statistics
are PAGE-GLOBAL for reflowable formats too, using a real multi-page markdown
document with genuine CSS-column pagination (not the PDF-shaped stub the
sibling suite uses, which never exercises this path): random vs. sequential
tap order, scroll/navigate-away-and-back, and a container-rect-only
perturbation (real layout frozen) all produce identical results.
