...
''')['identifier']
c.call('Page.navigate',url='http://127.0.0.1:8765/index.html');time.sleep(1)
...
js("toggleDictation();recognition.error('network');window.__count=__speech.length");time.sleep(.8)
check('system/network error never restarts','!dictation.wanted && __speech.length===__count')
js('toggleDictation();recognition.end();stopBackgroundActivity();window.__count=__speech.length');time.sleep(1.4)
...
time.sleep(1.3);js('recognition.end()');time.sleep(2.5);js('recognition.end()');time.sleep(3.1);js('recognition.end()');settle()
...
js("els.grammarPanel.classList.remove('expanded','loading','ready');els.askPanel.classList.remove('expanded','loading','ready');stopOnboarding();onboardingState={};localStorage.removeItem(ONBOARDING_KEY);state.bookKey='learning-fixture';state.format='pdf';state.currentIndex=1;scheduleReaderOnboarding()")
time.sleep(.8)
check('three soft onboarding cycles','getComputedStyle(els.askTab).animationIterationCount==="3" && els.askTab.classList.contains("onboarding-cue") && els.menuHandle.classList.contains("onboarding-cue")')
...
js("els.askPanel.classList.remove('loading');els.grammarTab.click();stopOnboarding();state.currentIndex=2;scheduleReaderOnboarding()");time.sleep(.8)
...
js("els.grammarPanel.classList.remove('expanded');onboardingState={};state.currentIndex=1;scheduleReaderOnboarding()");time.sleep(.8)
check('reduced motion gets static cue only',"els.askTab.classList.contains('onboarding-static') && getComputedStyle(els.askTab).animationName==='none'")
time.sleep(5)
