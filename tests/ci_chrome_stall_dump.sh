...
echo '--- crash dumps ---'
find /tmp/pdf-ux-test-profile "$HOME/.config/google-chrome/Crash Reports" -name '*.dmp' -print0 2>/dev/null |
  while IFS= read -r -d '' d; do echo "$d: $(strings -n 3 "$d" | grep -m1 -A1 -x ptype | tr '\n' ' ')"; done
