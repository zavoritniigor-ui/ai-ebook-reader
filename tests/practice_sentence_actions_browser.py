...
c.js("window.__tapSeen=[]; document.addEventListener('pointerdown', e => __tapSeen.push(e.pointerType), {capture: true, once: true}); 1")
...
# later without anything being wrong. A tap that never starts speech still fails below (with __tapSeen).
for _ in range(30):
...
if not c.js("document.querySelector('.practice-speak-btn').classList.contains('speak-active')"):
    diag = c.js("""(()=>{const b=document.querySelector('.practice-speak-btn'); const e=document.elementFromPoint(%f,%f);
        return { at:[%f,%f], under: e?(e.tagName+'.'+e.className+' '+(e.textContent||'').slice(0,40)):null,
          ...
          speakCalls: window.__ttsSpeakCalls.length, viewport:[innerWidth, innerHeight], pointerdownSeen: window.__tapSeen }; })()""" % (btn_rect['x'], btn_rect['y'], btn_rect['x'], btn_rect['y']))
